# Investigation Notes — Lab 61 Suspicious Proxy Configuration Investigation

## Investigation Overview

The investigation examined Windows proxy configuration from a DFIR perspective. The primary goal was to establish the normal state of the endpoint, simulate a suspicious-looking user proxy configuration, correlate the change with available telemetry, and document the limitations of the available evidence.
...
1. User-level proxy configuration was initially disabled.
2. WinHTTP was configured for direct access.
3. No proxy environment variables were identified.
4. No AutoConfigURL was identified.
5. A loopback proxy value was used for controlled testing.
6. The proxy string was mistakenly assigned to `ProxyEnable`.
7. `ProxyServer` was later cleared.
8. Sysmon Event ID 1 was available.
9. Sysmon Event ID 3 was available.
10. Relevant PowerShell Event ID 4104 evidence was not found.
11. Relevant Security Event ID 4688 evidence was not found.
12. No malicious proxy use was established.

## Analyst Assessment

The controlled exercise demonstrated how proxy settings can be investigated as endpoint configuration artifacts.

The strongest evidence came from the before-and-after Registry values rather than from network events.

The lab also demonstrated that configuration errors can create misleading data. Therefore, analysts must validate property semantics before interpreting a Registry value as evidence of malicious behavior.

## Final Investigation Conclusion

The investigation established a controlled modification to user Internet Settings but did not establish malicious intent or actual traffic redirection through the simulated proxy.

The main investigative lesson is:

> Configuration state, process evidence, and network behavior must be correlated before treating a proxy change as a security incident.
