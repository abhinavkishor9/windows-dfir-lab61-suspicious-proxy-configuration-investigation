# Troubleshooting Notes — Lab 61 Suspicious Proxy Configuration Investigation
...
| Issue | Resolution |
|---|---|
| ProxyEnable/ProxyServer confusion | Documented property semantics |
| Proxy string written to ProxyEnable | Cleared ProxyServer and flagged final-state verification |
| WinHTTP remained direct | Treated as separate configuration mechanism |
| No proxy environment variables | Documented baseline |
| No AutoConfigURL | Documented baseline |
| 4104 no relevant result | Documented telemetry gap |
| 4688 no relevant result | Documented telemetry gap |
| Sysmon 1 available | Used as supporting process evidence |
| Sysmon 3 available | Used for network context |
| No actual proxy server | Kept lab isolated |

## Final Troubleshooting Lesson

The most important lesson from Lab 61 is:

> **A proxy configuration investigation starts with configuration semantics and baseline state before moving to maliciousness.**

A strong investigation should distinguish:
...
What process changed it?
        |
        v
Did applications actually use it?
        |
        v
What network behavior followed?
```

Without the later links, a proxy change remains an investigation lead rather than proof of compromise.
